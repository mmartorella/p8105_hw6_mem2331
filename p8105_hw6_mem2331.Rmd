...
The residuals are evenly centered around 0, suggesting a linear relationship between the variables and baby weight. However there is possible heteroskedasticity, as baby weight may diverge more from predicted values at low baby weights.
...
Create models to predict birthweight:
  ...
  2. `all_baby` - head circumference, length, sex, and all interactions of these variables
...
Now comparing `all_baby` and `basic_baby` to `molly_model` in terms of cross-validated prediction error: (chose 100 because that's what we did in class).
...
From the plot it is apparent that the model incorporating the most baby traits and interaction of those traits is the best model for predicting baby weight (the `all_baby` model). The `molly_model` suffers likely because it primarily uses characteristics of the mother to predict the weight of the baby.
...
Apply linear model with tmax as the response and tmin as the predictor to 5000 bootstrapped samples of `weather_df`:
...
Now to extract r squared and calculate the log of the product of beta hat 1 and beta hat 0 for each bootstrapped sample.
...
The `r_squared` values are centered around ~0.912, which is very close to 1, indicating that the majority of the variance in tmax can be explained by the value of tmin. The distribution does appear to be skewed slightly to the left, which is likely a result of the values being so close to the maximum of 1 that the distribution cannot be exactly symmetric.

The 95% confidence interval for the `r_squared` values is (`r round(quantile(pull(estimates, r_squared), probs = c(0.025,0.975)), digits = 3)`).

### log(b0*b1)

Plotting the distribution of the log of the product of the beta hat values shows:
...
The 95% confidence interval for the `log_b1_b0` values is (`r round(quantile(pull(estimates, log_b1_b0), probs = c(0.025,0.975)), digits = 3)`).
